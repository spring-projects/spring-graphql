...
except there is no web framework in use, and `GraphQlService` is used to perform requests.
====
...
`GraphQlService` is used to perform the above test. It represents common infrastructure
for HTTP and WebSocket transports that both Spring WebMvc and WebFlux handlers delegate to.
Therefore, it does not involve a web framework and does not apply the `WebInterceptor`
