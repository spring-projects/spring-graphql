...
class GraphQlWebMvcSecurityAutoConfigurationTests {
	...
	void authenticatedUserShouldGetData() {
		testWith((mockMvc) -> {
			String query = "{" +
					...
					"  }" +
					"}";
			MvcResult result = mockMvc.perform(post("/graphql").content("{\"query\": \"" + query + "\"}").with(user("rob"))).andReturn();
			mockMvc.perform(asyncDispatch(result))
					.andExpect(status().isOk())
					.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
	...
	static class BookService {
		...
		@Nullable
		public Book getBookdById(String id) {
			return GraphQlDataFetchers.getBookById(id);
		}
